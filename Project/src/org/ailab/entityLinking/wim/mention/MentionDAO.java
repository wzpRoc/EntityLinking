...
public class MentionDAO extends BaseDAO {
	...
	protected BaseDTO resultSetToDto(ResultSet res) throws SQLException {
		...
		dto.setInitialWeightInDoc(res.getDouble("initialWeightInDoc"));
	...
	protected String getSqlForInsert() {
		return "INSERT INTO mention (id, docId, seqInDoc, startIdx, endIdx, name, entityId, wikiTitle, initialWeightInDoc, tf, idf, tfidf)VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
	}
	...
	protected String getSqlForUpdate() {
		return "UPDATE mention SET docId = ?, seqInDoc = ?, startIdx = ?, endIdx = ?, name = ?, entityId = ?, wikiTitle = ?, initialWeightInDoc = ?, tf = ?, idf = ?, tfidf = ? WHERE id = ?";
	}
    ...
    public List<Mention> getListByDocId(int docId) {
        return getListByCondition("docId=? and entityId>0", docId);
    }

    /**
     * 根据docId获得mention列表
     */
    public List<Mention> getListByDocId(int docId, int maxMentions) {
        String condition = "docId=? and entityId>0";
        if(maxMentions>0) {
            condition+=" limit "+maxMentions;
        }
        return getListByCondition(condition, docId);
    }
