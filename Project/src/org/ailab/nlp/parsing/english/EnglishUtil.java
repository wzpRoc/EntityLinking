...
public class EnglishUtil {
    ...
    public synchronized static List<SentenceAnnotation> splitSentence(String text) throws Exception {
        ...
        for (Annotation sentenceAnno : sentenceAnnoList) {
            SentenceAnnotation sentenceAnnotation = new SentenceAnnotation(sentenceAnno.getStartNode().getOffset().intValue(), sentenceAnno.getEndNode().getOffset().intValue());
    ...
    public static void createController() throws Exception {
        if (controller == null) {
            ...
            // 词性
            controller.add((ProcessingResource) Factory.createResource(PRNames.POS_TAGGER, Factory.newFeatureMap()));
        }
    }
    ...
    public static void main(String[] args) throws Exception {
        List list = parseSenToken("I love you!");
        for(Object sen: list) {
            for(TokenAnnotation token:((SentenceAnnotation) sen).tokenAnnotationList) {
                System.out.println(token.getText() + "\t"+token.getType() + "\t"+token.getFeature("category"));
            }
        }
    }
}
