...
public class WikiFactDAO extends BaseDAO {
	...
	protected BaseDTO resultSetToDto(ResultSet res) throws SQLException {
		...
		dto.setSubjectId(res.getInt("subjectId"));
		dto.setPredicateId(res.getInt("predicateId"));
		dto.setObjectId(res.getInt("objectId"));
		dto.setSubjectName(res.getString("subjectName"));
	...
	protected String getSqlForInsert() {
		return "INSERT INTO wikiFact (id, subjectId, predicateId, objectId, subjectName, predicateName, objectName)VALUES(?, ?, ?, ?, ?, ?, ?)";
	}
	...
	protected void setParametersForInsert(List<Object> paramList, BaseDTO baseDTO) {
		...
		paramList.add(dto.getSubjectId());
		paramList.add(dto.getPredicateId());
		paramList.add(dto.getObjectId());
		paramList.add(dto.getSubjectName());
	...
	protected String getSqlForUpdate() {
		return "UPDATE wikiFact SET subjectId = ?, predicateId = ?, objectId = ?, subjectName = ?, predicateName = ?, objectName = ? WHERE id = ?";
	}
